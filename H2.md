...
Jatkoin 6.11.2024 klo 16.15 ja lopetin klo 

Tässä tapahtui yhtä jos toista ja jouduin aloittamaan alusta. 
...
Normaalisti voisi  kait käyttää brewta apuna, mutta sekään ei toimi, vaikka vielä päivitin sen. Nyt tulikin uusi tenkkapoo, sillä Salt projekti Blogissa (2024) sanotaan, että bootstrap scriptit tulee jatkossa hakea suoraan Githubin reposta. Eli lähden nyt sitä kokeilemaan Salt Projekt Blogin avustuksella. 

Mutta....ideat loppuivat täysin kesken, joten latasin saltin macille ja käytin latausvaiheessa saamani polun terminaalissa, jolloin salt latautui. 

<img width="622" alt="Näyttökuva 2024-11-05 kello 20 46 40" src="https://github.com/user-attachments/assets/09f06cd0-422f-4595-8d2a-5bf7dffba1c3">

<img width="582" alt="Näyttökuva 2024-11-05 kello 20 47 32" src="https://github.com/user-attachments/assets/bc417398-a7ca-4176-94f4-654e029335d7">

Lataustilanteessa kehoitti vielä updatemaan pipin, jonka tein komennolla:

    $ sudo /opt/salt/bin/../bin/python3.10 -m pip install --upgrade pip

<img width="583" alt="Näyttökuva 2024-11-05 kello 20 54 41" src="https://github.com/user-attachments/assets/656e3a3f-f6c6-400a-85bc-33a95fb0b51e">

Saltin versio näkyy tässä: 

<img width="350" alt="Näyttökuva 2024-11-05 kello 20 55 14" src="https://github.com/user-attachments/assets/5c0ff6a6-0d7f-46fc-86f3-a7f5d0e5ae85">

Ja nyt aloin tietysti miettimään, että pitikö se salt tosiaan myös ladata tähän isäntäkoneelle. Noh, se jää nähtäväksi. 

Noniin, ja sitten itse tehtävään. Tarkoituksena olisi siis luoda Herra-orja arkkitehtuuri tähän uuteen verkkoon ja demonstroida sitä. Asennan siis SSH1 koneelle salt-masterin ja SSH2 koneelle salt-minion. Laitan lisäksi orjan /etc/salt/minion -tiedostoon masterin osoite. Hyväksy avain ja osoita, että herra voi komentaa orjakonetta.

Aloitin menemällä avaamalla t001 koneen ssh:lla komennolla

    $ vagrant up
    $ vagrant ssh t001

Sitten päivitin kaikki ohjelmat ja asensin curlin komennoilla 
...
Lähteet: Karvinen 2023, Karvinen 2021, Salt projektin blogi 2024. 
